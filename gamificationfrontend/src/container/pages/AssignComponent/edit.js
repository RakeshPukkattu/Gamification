...
const EditAssign = () => {
  ...
  const getOneCompany = () => {
    const user = getItem('user');
    const api = process.env.REACT_APP_BACKEND_API;
    const URL = `${api}companyManagement/company/${id}`;
    var config = {
      method: 'get',
      url: URL,
      headers: {
    ...
    axios(config)
      .then(function(response) {
        ...
        let games = [];
        let themes = [];
        result.assignedComponents.components.map(com => {
          components.push(com.id);
        });
        result.assignedComponents.games.map(game => {
          games.push(game.id);
        });
        result.assignedComponents.theme.map(th => {
          themes.push(th.id);
        });
        setState2({ ...state2, selectedRowKeys: components });
        setState3({ ...state3, selectedRowKeys: games });
        setState4({ ...state4, selectedRowKeys: themes });
        setIndustry(result.assignedComponents.industries);
        setFlag(true);
      })
      .catch(function(error) {
        console.log(error);
  ...
  useEffect(() => {
    console.log('vfv', industries1, 'fvfv', industry);
    industries1.map(ind => {
      if (ind.industryName === industry[0].industryName) {
        setIndustries(ind);
        setThemes(ind.themes);
        setGames(ind.games);
      }
    });
  }, [flag]);
  ...
  const handleOk = () => {
    ...
    const user = getItem('user');
    const api = process.env.REACT_APP_BACKEND_API;
    const componentValues = { componentId: companys, gameId: games, themeId: themes[0] };
    var data = new FormData();
    data.append('userJson', JSON.stringify(componentValues));
    const URL = `${api}assignCompanyComponents/updateAssignedComponents/${id}`;
    var config = {
      method: 'put',
      url: URL,
      headers: {
      ...
      .then(function(response) {
        ...
        if (result.code.includes('406')) {
          ...
          history.push({
            ...
            state: { detail: `Components Updated Successfully` },
          });
        }
      })
      .catch(function(error) {
  ...
  return (
    <>
      <PageHeader title="Edit Assign Components" />
      <Main>
                        ...
                        <BasicFormWrapper className="basic-form-inner" style={{ marginTop: '-100px' }}>
                          <div className="">
                            <Row justify="center" style={{ margin: 'auto' }}>
                              <Col sm={22} xs={24}>
                                <div className="shipping-form">
                                  <Heading as="h4">3. Select Games</Heading>
                                </div>
                              </Col>
                            </Row>
                            <UserTableStyleWrapper>
                            ...
                            <Row justify="center">
                              <Col sm={22} xs={24}>
                                <div className="payment-method-form">
                                  <Heading as="h4">4. Verify the Components, Games and Theme</Heading>
                                </div>
                              </Col>
                            </Row>
                            <Row gutter={25}>
