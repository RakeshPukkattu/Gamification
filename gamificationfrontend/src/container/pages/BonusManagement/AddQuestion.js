...
import { WizardWrapper, Wizard, WizardBlock } from '../SurveyManagement/Style';
...
const AddQuestion = () => {
  ...
  useEffect(() => {
    ...
    setTotalQuestions(bonus.questions);
  }, []);
    ...
    merged.map(question => {
      ...
      if (question.questionOptions.length === 2) {
        questions.push({
          ...
          option3: '',
          option4: '',
          answer: answer.push(question.answer),
        });
  ...
  useEffect(() => {
    ...
    // }
    if (questionDetail === '' || fields.length === 0 || answer === '') {
      setFlag3(true);
  ...
  return (
    <>
      <PageHeader title="Add Bonus Questions" />
      <Main>
      ...
      <Modal title="Error in Deleting Questions" visible={isModalVisible2} onOk={handleOk2} onCancel={handleCancel}>
        <p>A Bonus must have atleast one question.</p>
      </Modal>
    </>
  );
};
