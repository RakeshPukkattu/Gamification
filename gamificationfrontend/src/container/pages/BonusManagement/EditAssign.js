...
const Assign = () => {
  ...
  const handleSubmit = values => {
    ...
    const URL = `${api}surveyManagement/modifyAssignedSurvey/${id}`;
    var config = {
      method: 'put',
      url: URL,
      headers: {
      ...
      .then(function(response) {
        ...
        if (result.code.includes('406')) {
          ...
          history.push({
            ...
            state: { detail: `Survey Assigned Updated Successfully` },
          });
        }
      })
  ...
  const getAssignedUsers = () => {
    ...
    axios(config)
      .then(function(response) {
        ...
        result.assignedGroups.map(groups => {
          groupsTableData2.push({
            key: groups.groupID,
            name: groups.groupName,
          });
        });
        let merged2 = groupsTableData2.concat(selectedGroups);
        setSelectedGroups(merged2);
        setReference2(merged2);
        setFlag5(true);
  ...
  useEffect(() => {
    const sorted2 = _.differenceBy(allGroups, selectedGroups, 'key');
    setAllGroups(sorted2);
    setReference(sorted2);
  }, [flag5]);
  ...
  return (
    <>
      <PageHeader title="Edit Assign Bonus Questions" />
      <Main>
        <Row gutter={25}>
          <Col sm={24} xs={24}>
            <ExportStyleWrap>
              <Cards headless>
                ...
                <Form name="adduser" form={form} onFinish={handleSubmit} layout="vertical">
                  <Radio.Group onChange={selectAssign} value={value} style={{ marginBottom: '20px' }}>
