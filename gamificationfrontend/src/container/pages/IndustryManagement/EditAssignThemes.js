...
const EditAssignThemes = () => {
      ...
      .then(function(response) {
        ...
        if (result.code.includes('406')) {
          ...
          history.push({
            ...
            state: { detail: `Themes Assigned Updated Successfully` },
          });
        }
      })
  ...
  useEffect(() => {
    const sorted = _.differenceBy(allThemes, selectedThemes, 'key');
    setAllThemes(sorted);
    setReference(sorted);
  }, [flag]);
  ...
  return (
    <>
      <PageHeader title="Edit Assign Themes" />
      <Main>
                ...
                <Form name="adduser" form={form} onFinish={handleSubmit} layout="vertical">
                  ...
                  <div className="sDash-button-grp">
                    <Button
                      ...
                      disabled={loading}
                    >
                      {loading2 ? <Spin size="medium" /> : 'Update Assign Themes'}
                    </Button>
                  </div>
                </Form>
                <div>
...
export default EditAssignThemes;
