...
const Dashboard = () => {
  ...
  const getAllSessionsInCountry = (countryTemp) => {
      const api = process.env.REACT_APP_BACKEND_API;
      const URL = `${api}gameSessionManagement/gameSession/sessionInCountry/${countryTemp}`;
      console.log(URL);
      var config = {
        method: 'get',
        url: URL,
              ...
              modify: (
                <>
                  <Button
                    ...
                    onClick={() => {
                      history.push(`/admin/editsession/${sessionId}`);
                    }}
                  >
  ...
  const getAllAdmins = () => {
    ...
    const getAllSessions = () => {
      ...
      const api = process.env.REACT_APP_BACKEND_API;
      const URL = `${api}gameSessionManagement/gameSessions`;
      var config = {
        method: 'get',
        url: URL,
        headers: {
  ...
  const updateStatus = (e, id, name) => {
    const user = getItem('user');
    const api = process.env.REACT_APP_BACKEND_API;
    const URL = `${api}gameSessionManagement/updateEnableDisable/${id}`;
    const userValues = {
  ...
  return (
    <>
      <PageHeader title="Session Management Dashboard" />
      <Main>
        <Row gutter={25}>
          <Col sm={24} xs={24}>
            <ExportStyleWrap>
      ...
      {singleSession === null ? (
        ...
        >
          ...
          {singleSession.getGroupsInSession.map(group => {
            return (
              <>
                <Collapse accordion style={{ marginTop: '10px' }}>
                  <Panel header={group.groupName}>
                    <UserTableStyleWrapper>
                      <TableWrapper className="table-responsive">
                        <Table id="myTable" scroll={{ y: 240 }} dataSource={group.groupMembers} pagination={false}>
                          <Column
                            title="Name"
                            dataIndex="userName"
                            key="userName"
