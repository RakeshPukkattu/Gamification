...
const EditQuestion = () => {
  ...
  const getOneSurveys = () => {
    ...
    axios(config)
      .then(function(response) {
        ...
        let dropdown = 0;
        let Questions = [];
        ques.questions.map(questionss => {
          if (questionss.questionType === 'MCQ') {
            mcq = mcq + 1;
            const { id, questionNumber, questionStem, options, answer } = questionss.question;
            let gg = options.split(',');
            let ans = 0;
            let ss = [];
            if (gg.length === 2) {
              ss = [{ value: gg[0] }, { value: gg[1] }];
            }
            if (gg.length === 3) {
              ss = [{ value: gg[0] }, { value: gg[1] }, { value: gg[2] }];
            }
            if (gg.length === 4) {
              ss = [{ value: gg[0] }, { value: gg[1] }, { value: gg[2] }, { value: gg[3] }];
            }
            if (gg.length === 5) {
              ss = [{ value: gg[0] }, { value: gg[1] }, { value: gg[2] }, { value: gg[3] }, { value: gg[4] }];
            }
            gg.map((g, index) => {
              if (g === answer) {
                ans = index + 1;
            ...
            Questions.push({
              questionNumber: questionss.questionNumber,
              questionDetail: questionStem,
              mcqOptions: ss,
              mmcqOptions: [{ value: null }, { value: null }],
              mcqAnswer: ans,
              mmcqAnswer: [],
              dropdownOptions: [{ value: null }, { value: null }],
              labelOptions: [{ value: null }, { value: null }],
              dropdownAnswer: [{ value: null }, { value: null }],
            });
          } else if (questionss.questionType === 'MMCQ') {
            mmcq = mmcq + 1;
            const { id, questionNumber, questionStem, options, answers } = questionss.question;
            let ss = [];
            let ans = [];
            let gg = options.split(',');
            if (gg.length === 2) {
            ...
            Questions.push({
              questionNumber: questionss.questionNumber,
              questionDetail: questionStem,
              mcqOptions: [{ value: null }, { value: null }],
              mmcqOptions: ss,
              mcqAnswer: '',
              mmcqAnswer: ans,
              dropdownOptions: [{ value: null }, { value: null }],
            ...
            dropdown = dropdown + 1;
            const { id, questionNumber, questionStem, answers, labels, options } = questionss.question;
            let answer = [];
            let label = [];
            let option = [];
            answers.split(',').map(an => {
              answer.push({ value: an });
            });
            labels.split(',').map(an => {
              label.push({ value: an });
            });
            options.split(',').map(an => {
              option.push({ value: an });
            });
            Questions.push({
              questionNumber: questionss.questionNumber,
              questionDetail: questionStem,
        ...
        setNoOfMCQ(mcq);
        setNoOfMMCQ(mmcq);
        setNoOfDropdown(dropdown);
        setAllQuestions(Questions);
        let aa = mcq + mmcq + dropdown;
        setTotalQuestions(aa);
      })
  ...
  useEffect(() => {
    if (allQuestions.length !== 0) {
      allQuestions.map(question => {
        if (parseInt(question.questionNumber) === current) {
  ...
  const next = () => {
    if (flag3 === true) {
      ...
      if (current >= allQuestions.length) {
        ...
        setShow2(false);
        setFlag(true);
      } else if (parseInt(allQuestions[current].questionNumber) === current) {
        console.log('ssss');
  ...
  const handleOk = () => {
    ...
    let merged = [];
    let aa = parseInt(noOfDropdown) + parseInt(noOfMCQ) + parseInt(noOfMMCQ);
    if (totalQuestions == aa) {
      allQuestions[current].questionDetail = questionDetail;
      allQuestions[current].mcqOptions = mcqOptions;
      allQuestions[current].mmcqOptions = mmcqOptions;
      allQuestions[current].mcqAnswer = mcqAnswer;
      ...
      merged = allQuestions.concat(question);
      setAllQuestions(merged);
    }
    // let merged = allQuestions.concat(question);
    // setAllQuestions(merged);
      ...
      .then(function(response) {
        ...
        if (result.code.includes('400')) {
          ...
          history.push({
            ...
            state: { detail: `${survey.name} Updated Successfully` },
          });
        }
      })
      .catch(function(error) {
                                    ...
                                    <Col sm={20} xs={24}>
                                      ...
                                      >
                                        {dropdownOptions.map(drop => (
                                          <Select.Option value={drop.value}>{drop.value}</Select.Option>
                                        ))}
                                      </Select>{' '}
                                    </Col>
                                  </Row>
  ...
  //   if (questionDetail === '') {
  //     setFlag3(true);
  //     setError('Please add question detail');
  //   }
  // }, [questionDetail]);
  ...
  //     if (mcqAnswer === '') {
  //       setFlag3(true);
  //       setError('Please add question detail, options and mcqAnswer');
  //     } else {
  ...
  //       setFlag3(true);
  //       setError('Please add answer');
  //     } else {
  //       const found = mmcqOptions.some(row => row.value === null);
  //       if (found) {
  //         setFlag3(true);
  //         setError('Please add question detail, options');
  //       } else {
  //         setFlag3(false);
  //       }
  //     }
  ...
  //     setFlag3(false);
  //   }
  // }, [dropdownOptions, labelOptions, dropdownAnswer]);
  ...
  return (
    <>
      <PageHeader title="Edit Questions to the Survey" />
      <Main>
        <div className="wizard-side-border">
          <WizardBlock>
            <Cards headless>
              <div style={{ textAlign: 'left', marginBottom: '25px', marginTop: '-30px', marginLeft: '40px' }}>
                <Button
                  className="btn-signin"
