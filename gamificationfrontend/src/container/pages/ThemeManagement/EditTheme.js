...
const EditTheme = () => {
  ...
  useEffect(() => {
    ...
    if (theme !== null) {
      ...
      setImage2(theme.imageKey);
      let keywords = [];
      const kk = theme.keyWords.split(',');
      kk.map(key => {
        keywords.push({
          label: key,
          value: key,
  ...
  const handleSubmit = values => {
    ...
    const user = getItem('user');
    const api = process.env.REACT_APP_BACKEND_API;
    const gameValues = { themeName: values.themeName, keywords: keywords };
    ...
    const URL = `${api}Themes/modifyTheme/${id}`;
    var config = {
      method: 'put',
      url: URL,
      headers: {
      ...
      .then(function(response) {
        ...
        if (result.code.includes('406')) {
          ...
          history.push({
            ...
            state: { detail: `${values.themeName} Updated Successfully` },
          });
        }
      })
      .catch(function(error) {
          ...
          <Col sm={24} xs={24}>
            <ExportStyleWrap>
              <Cards headless>
                ...
                <Form name="editheme" form={form} onFinish={handleSubmit} layout="vertical">
                  <Form.Item
                    name="themeName"
                    rules={[{ message: 'Please enter theme name', required: true }]}
                    label="Theme Title"
                  >
                    <Input
